...
def setup_dota_gc_commands(bot: commands.Bot) -> None:
    ...
    async def create_dota_lobby(
        ...
        """Cria um lobby no Dota 2 via Game Coordinator.

        Uso: !criarlobby [nome] [senha] [modo] [regiao]
        Exemplo: !criarlobby "Liga Fumos" "fumos123" captains sa
        """
        gc = get_gc_manager()
        if gc is None:
            await ctx.send(
        ...
        if modo.lower() not in GAME_MODES:
            await ctx.send(
                f"❌ Modo inválido `{modo}`. Opções: {_MODO_OPCOES}",
                delete_after=12,
            )
            return
        ...
        if regiao.lower() not in SERVER_REGIONS:
            await ctx.send(
                f"❌ Região inválida `{regiao}`. Opções: {_REGIAO_OPCOES}",
                delete_after=12,
            )
            return
        ...
        if lobby is None:
            await status_msg.edit(
                content="❌ Falha ao criar o lobby. O GC não respondeu a tempo — tente novamente."
            )
            return
        ...
        logger.info(
            f"[GC] Lobby criado por {ctx.author} | nome={lobby.name} | modo={modo} | regiao={regiao}"
        )

    @bot.command(name="fecharlobby", aliases=["deletarlobby", "closelobby", "gcfechar"])
    async def destroy_dota_lobby(ctx: commands.Context):
        """Encerra o lobby Dota 2 ativo. Apenas administradores."""
        if not is_admin(ctx.author.id):
            await ctx.send(
                "❌ Apenas administradores podem fechar o lobby.", delete_after=8
            )
            return
    ...
    async def gc_status(ctx: commands.Context):
        """Exibe o status do Game Coordinator e do lobby ativo."""
        gc = get_gc_manager()

        embed = discord.Embed(
            title="📡 Dota 2 Game Coordinator", color=discord.Color.blue()
        )
        ...
        gc_status = "🟢 Conectado" if gc.is_ready else "🔴 Desconectado / conectando..."
        embed.add_field(name="GC", value=gc_status, inline=False)
        ...
        if gc.current_lobby:
            ...
            embed.add_field(
                name="Modo", value=GAME_MODE_LABELS.get(lb.game_mode, lb.game_mode), inline=True
            )
            embed.add_field(
                name="Servidor", value=REGION_LABELS.get(lb.server_region, lb.server_region), inline=True
            )
            if lb.members:
                embed.add_field(
            ...
            if gc.is_ready:
                embed.set_footer(text=f"Use !criarlobby para criar um lobby  |  modos: {_MODO_OPCOES}")
